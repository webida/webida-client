...
define([
    ...
    'plugins/workbench/preference-system/store',
    'src/util/keyCode',
    'other-lib/toastr/toastr'
], function(Deferred, when, all, _, prefStore, keyCode, toastr) {
    'use strict';
    ...
    function checkWholeItems(contr) {
        ...
        if (contr.location === null) {
            ...
            if ( //ownKeys.length === 0 ||
                ownKeys.some(function(key) {
                    return contr.wholeItems[key][0] !== 'cmnd';
                })) {
                return 'Plugin ' + contr.__plugin__.manifest.name +
                    ' registers menu items with an invalid "wholeItems" property:' +
    ...
    function normalizeKeysStr(str) {
        ...
        arr = arr.slice(0, -1).map(function(elmt) {
            return elmt.toUpperCase();
        }).sort();

        if (arr.every(function(elmt) {
            return modKeyStr.indexOf(elmt) > -1;
        })) {
            var arr2 = [];
                ...
                _.each(items, function(value, key) {
                    ...
                    if (hasPredefinedHierarchy) {
                        ...
                        if (isDelim) {
                            ...
                            addItemsToTargetInner(newLocation + '/', (target[key] = {
    ...
    function getCmdsStr(shortcutItems) {
        var paths = shortcutItems.map(function(item) {
            return item.path;
        });
        var cmdNames = paths.map(function(path) {
            return itemPathToCmdName(path);
        });
    ...
    function MenuItemTree(predefinedHierarchy, contributors, topElem, pluginName, options) {
        ...
        topElem.addEventListener('keydown', function(event) {
            ...
            if ((shortcutItems = self._wholeShortcutItems[keys])) {
                ...
                var keepDefault = shortcutItems.every(function(scItem) {
                    return scItem.keepDefault;
                });
                var propagate = shortcutItems.every(function(scItem) {
                    return scItem.propagate;
                });
                var skipInvoc = shortcutItems.every(function(scItem) {
                    return scItem.skipInvoc;
                });
                if (!keepDefault) {
    ...
    function onShortcutChanged(menuItemTree, restoring) {
        ...
        if (!restoring) {
            var changed = {},
                delta = getShortcutDelta(menuItemTree._wholeItems);
