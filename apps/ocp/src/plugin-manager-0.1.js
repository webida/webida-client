        ...
        require(dependency, function() {
            function parseRequirement(str) {
                return str.split('|').map(function(c) {
                    return c.trim().split('&').map(function(d) {
                        return d.trim();
                    }); // disjuncts
                });
            }
            ...
            manifests.forEach(function(manifestStr, i) {
                ...
                if (manifest.requirement) {
                    ...
                    if (_.flatten(manifest.requirement).some(function(a) {
                        return !a;
                    })) {
                        alert('Error: A plugin (' + manifest.name +
            ...
            Object.keys(wholePlugins).forEach(function(pluginName) {
                var plugin = wholePlugins[pluginName],
                    eps;
            ...
            all(promises).then(function(results) {
                ...
                nonLocalPlugins.forEach(function(pluginName) {
                    // var configFile = URI(appPath).segment('plugins/' + pluginName + '/config.json').path();
                    var configFile = 'plugins/' + pluginName + '/config.json';
        ...
        initPlugins: function(nextJob) {
            ...
            function trimPluginSettings(settings) {
                ...
                pluginSettingsProperties.forEach(function(p) {
                    if (settings[p]) {
                        trimed[p] = settings[p].map(function(str) {
                            return str.trim();
                        });
                    }
                });
            ...
            function readAppPluginSettings() {
                ...
                var settingsFile = 'plugins/plugin-settings.json';
