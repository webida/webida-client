...
(function(mod) {
  ...
  function updateText(file, text, srv) {
    if (typeof text === 'object') {
      ...
      if (model.version !== model.astVersion) {
        model.ast = infer.parse(model.text, srv.passes);
  ...
  Server.prototype = signal.mixin({
    ...
    checkUpdates: function() {
      if (this.fileServer) {
        for (var i = 0; i < this.files.length; i++) {
          var file = this.files[i];
          var filemodel = this.fileServer.getUpdatedFile(file.name, file.scopeVersion);
          if (filemodel) {
            ensureFile(this, file.name, filemodel);
          }
        }
      }
    },
  ...
  function analyzeFile(srv, file) {
    infer.withContext(srv.cx, function() {
      ...
      if (file.model) {
        file.scopeVersion = file.astVersion;
      }
  ...
  function resolveFile(srv, localFiles, name) {
    ...
    infer.withContext(srv.cx, function() {
      ...
      tieTogether: if (inObject || atFunction) {
        ...
        var newInner = infer.scopeAt(file.ast, bodyStart, scopeStart);
        if (!newInner.fnType) break tieTogether;
        if (inObject) {
          var prop = inObject.type.getProp(inObject.prop);
          prop.addType(newInner.fnType);
        } else if (atFunction) {
          var inner = infer.scopeAt(realFile.ast, pos + bodyStart, realFile.scope);
  ...
  function findCompletions(srv, query, file) {
    ...
    function gather(prop, obj, depth) {
      ...
      if (val && val.originNode && val.originNode.start === wordStart && val.originNode.end === wordEnd) {
        return;
      }
...
//        var val = obj ? obj.props[prop] : infer.ANull;
  ...
  function buildRename(srv, query, file) {
    ...
    function qsort(list, comp) {
      if (list.length <= 1) return list;
      else {
        var lesser = [], greater = [];
        for (var i = 1; i < list.length; i++) {
          if (comp(list[i], list[0]) <= 0) {
            lesser.push(list[i]);
          } else {
            greater.push(list[i]);
          }
        }
        lesser = qsort(lesser, comp);
        greater = qsort(greater, comp);
        lesser.push(list[0]);
        return lesser.concat(greater);
      }
    }
    refs = qsort(refs, function (a, b) {
      return -(function() {
        if (a.start.line < b.start.line) return -1;
        else if (a.start.line > b.start.line) return 1;
        else if (a.start.ch < b.start.ch) return -1;
        else if (a.start.ch > b.start.ch) return 1;
        else return 0;
      })();
    });
