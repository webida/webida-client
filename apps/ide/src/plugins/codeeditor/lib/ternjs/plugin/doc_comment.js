...
(function(mod) {
  ...
  if (typeof define == "function" && define.amd) // AMD
    return define(["../lib/infer", "../lib/tern", "../lib/comment", "acorn/util/walk", "webida-lib/util/path"], mod);
  mod(tern, tern, tern.comment, acorn.walk);
})(function(infer, tern, comment, walk, path) {
  "use strict";
  ...
  function postParse(ast, text) {
    ...
    walk.simple(ast, {
      ...
      ExpressionStatement: attachComments
    });
  }
  ...
  function postInfer(ast, scope) {
    walk.simple(ast, {
      ...
      ExpressionStatement: function(node, scope) {
        if (node.commentsBefore)
          interpretComments(node, node.commentsBefore);
      }
  ...
  function jsdocInterpretComments(node, scope, aval, comments) {
    ...
    for (var i = 0; i < comments.length; ++i) {
      ...
      var decl = /(?:\n|$|\*)\s*@(type|param|arg(?:ument)?|returns?|see)\s+(.*)/g, m;
      while (m = decl.exec(comment)) {
        // webida extension
        // add value of @see to content assist engine
        if (m[1] === "see") {
          var filepath = m[2].trim();
          if (path.endsWith(filepath, ['.js', '.json', '.html'])) {
            var server = infer.cx().parent;
            var currpath = infer.cx().curOrigin;
            var abspath = path.combine(path.getDirPath(currpath), filepath);
            server.addFile(abspath);
          }
        }
