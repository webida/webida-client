...
/**
 * @file Constructor function for HTML linter control
 *
 * @constructor
 * @since: 1.6.1
...
// @formatter:on
    ...
    function setLinter(viewer, type, option)
    {
        ...
        if (type !== HtmlLintControl.TARGET_MODE) {
            return;
        }
        ...
        if (option) {
            loadCSSList([
                ...
                require([
                    ...
                    require(['./html-lint'], function () {
                        CodeEditorViewer.addAvailable('addon', 'lint');
                        CodeEditorViewer.addAvailable('addon', 'html-lint');
    ...
    {
        if (editorMode !== HtmlLintControl.TARGET_MODE) {
            return;
        }
        
        if (CodeEditorViewer.isAvailable('addon', 'lint') && 
            CodeEditorViewer.isAvailable('addon', 'html-lint')) {
    ...
    function HtmlLintControl(viewer, cm, options, c) {
        logger.info('new HtmlLintControl()');   
    ...
    genetic.inherits(HtmlLintControl, IContentAssist, {        
