...
(function(mod) {
  ...
  if (typeof define == "function" && define.amd) // AMD
    return define(["../lib/infer", "../lib/tern", "webida-lib/util/path"], mod);
  mod(tern, tern);
})(function(infer, tern, pathUtil) {
  "use strict";
  ...
  function resolveName(name, data) {
    ...
    var base = opts.baseUrl || opts.maybeBaseUrl || "";
    var isRelative = /^\.?\.\//.test(name);
    if (isRelative) {
      base = pathUtil.getDirPath(data.currentFile);
    }
    if (base && base.charAt(base.length - 1) != "/") base += "/";
    if (opts.paths) {
      var known = opts.paths[name];
      if (known) return pathUtil.flatten(base + known + ".js");
      var dir = name.match(/^([^\/]+)(\/.*)$/);
      if (dir) {
        var known = opts.paths[dir[1]];
        if (known) return pathUtil.flatten(base + known + dir[2] + ".js");
      }
    }
    return pathUtil.flatten(base + name + ".js");
  }
  ...
  function getInterface(name, data) {
    ...
    name = pathUtil.flatten(name);
    var known = data.interfaces[name];
    if (!known) {
      known = data.interfaces[name] = new infer.AVal;
      var depth = data.depths[data.currentFile] + 1;
      if (data.options.maxDepth === undefined || depth <= data.options.maxDepth) {
        data.depths[name] = depth;
  ...
  infer.registerFunction("requireJS", function(_self, args, argNodes) {
    ...
    if (argNodes && args.length > 1) {
      ...
      out.addType(exports, EXPORT_OBJ_WEIGHT);
      fn = args[0];
    } else if (argNodes && args.length == 1 && argNodes[0].type == "Literal" && typeof argNodes[0].value == "string") {
      // example : var module = require('lib/module');
      return getInterface(argNodes[0].value, data);
    }
  ...
  infer.registerFunction("requireJSConfig", function(_self, _args, argNodes) {
    ...
    if (data && argNodes && argNodes.length && argNodes[0].type == "ObjectExpression") {
      ...
      for (var key in config) if (config.hasOwnProperty(key)) {
        ...
        if (!exists) {
          if (key === 'baseUrl' && value && value[0] !== '/') {
            value = pathUtil.getDirPath(infer.cx().curOrigin) + value;
          }
  ...
  tern.registerPlugin("requirejs", function(server, options) {
    ...
    server.on("beforeLoad", function(file) {
      if (this._requireJS.interfaces[file.name]) {
        this._requireJS.interfaces[file.name].types = [];
      }
      this._requireJS.currentFile = file.name;
      if (!this._requireJS.depths[file.name]) {
        this._requireJS.depths[file.name] = 0;
      }
    });
