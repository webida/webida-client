...
define(['monitorApi', 'js/data', 'toastr', 'moment'], function (monitorApi, dataMgr, toastr, moment) {
    ...
    function init() {
        ...
        function getQueryOptions() {
            ...
            var valOfSvcType = getSel('#cur-option-svctype');
            if (valOfSvcType !== 'All') {
                option.svc_type = valOfSvcType;
            }

            var valOfSvcName = getSel('#cur-option-svcname');
            if (valOfSvcName !== 'All') {
                option.inst_name = valOfSvcName;
            }

            var valOfIstid = getSel('#cur-option-instid');
            if (valOfIstid !== 'All') {
                var valsFromIstid = valOfIstid.split('|');
                option.inst_id = valsFromIstid[0];
            }

            var valOfUrl = getSel('#cur-option-url');
            if (valOfUrl !== 'All') {
