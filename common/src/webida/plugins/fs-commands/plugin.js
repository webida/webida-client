...
define([
    ...
    var module = {
        ...
        getViableItemsForWorkbenchAtFile: function () {
            var items = {
                '&Download Files': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                    'handleDownloadZip'
                ],
                'Duplica&te': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                    'handleDuplicate'
                ]
            };
            ...
            if (selPaths && selPaths.length > 0) {
                ...
                if (selPaths.every(function (path) {
                    ...
                    items['&Refresh'] = ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                        'handleRefresh'
                    ];
                    //items['&Upload Files'] =
                    // [ 'cmnd',
                ...
                if (selPaths.length === 1) {
                    ...
                    items['Ren&ame'] = ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                        'handleRename'
                    ];
                    ...
                    if (pathUtil.isDirPath(selPath)) {
                        ...
                        items['&New']['&Directory'] = ['cmnd',
                            'webida-lib/plugins/fs-commands/fs-commands',
                            'handleNewFolder'
                        ];
                    }
                }
                ...
                if (selPaths.every(function (path) {
                    ...
                    items['Open &File'] = ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                        'handleEdit'
                    ];
                }
            }
        ...
        getViableItemsForWorkbenchAtFind: function () {
            var items = {
                '&Find in Files': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                    'findInCurDir'
                ]
            };
        ...
        getViableItemsForWorkbenchAtNavigate: function () {
            var items = {
                'Go to &File': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                    'gotoFileInCurDir'
                ]
            };
        ...
        getViableItemsForWorkspaceView: function () {
            ...
            if (paths && paths.length > 0 && paths.every(function (n) {
                ...
                var availableExtensions = extensionManager.getExtensionsForType(pathUtil.getFileExt(
                    paths[0]));
                    ...
                    if (i > 0) {
                        if (availableExtensions) {
                            ...
                            if (availableExtensions2) {
                                availableExtensions = _.intersection(availableExtensions,
                                    availableExtensions2);
                            } else {
            ...
            var candidateItemSets = [
                ...
                // items for multiple selection
                {
                    ...
                    items: {
                        ...
                        'Dow&nload': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleDownloadZip'
                        ]
                    }
                }, {
                    ...
                    items: {
                        '&Copy': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleCopy'
                        ],
                        'Cu&t': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands', 'handleCut'],
                        '&Delete': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleDelete'
                        ]
                    }
                },
                ...
                // folder items
                {
                    ...
                    items: {
                        'New': {
                            '&File': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                                'handleNewFile'
                            ],
                            '&Directory': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                                'handleNewFolder'
                            ]
                        },
                        'Dow&nload': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                    ...
                    isApplicableTo: function (paths) {
                        ...
                        return paths.length === 1 && pathUtil.isDirPath(paths[0]) &&
                            (pathUtil.getLevel(paths[0]) !== 1);
                    },
                    items: {
                        //'Properti&es':
                        // [ 'cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                        // 'handleFileProperties' ],
                        'Dup&licate': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleDuplicate'
                        ],
                        '&Copy': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleCopy'
                    ...
                    items: {
                        '&Paste': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handlePaste'
                        ]
                    }
                }, {
                    isApplicableTo: function (paths) {
                        return paths.length === 1 && pathUtil.isDirPath(paths[0]) && pathUtil.getLevel(
                                paths[0]) !==
                            1;
                        // folder && not workspace && not project
                    },
                    items: {
                        ...
                        'Ren&ame': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleRename'
                        ]
                    }
                }, {
                    ...
                    items: {
                        ...
                        // 'webida-lib/plugins/fs-commands/fs-commands-upload',
                        // 'handleUploadFiles' ],
                        'Refres&h': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'handleRefresh'
                        ],
                        '&Find in Files': ['cmnd', 'webida-lib/plugins/fs-commands/fs-commands',
                            'findInCurDir'
