...
// @formatter:on
    ...
    genetic.inherits(PartContainer, EventEmitter, {
        ...
        createPart: function(PartClass, callback) {
            ...
            //3. Create Viewer
            var createViewer = new Promise(function(resolve, reject) {
                try {
                    var viewer = part.createViewer(that.getContentNode());
                    viewer.once(Viewer.READY, function(viewer) {
                        if ( viewer instanceof Viewer) {
                            logger.info('%cViewer.READY', 'color:red');
                            resolve(viewer);
                        }
                    });
                } catch(e) {
            ...
            //4. Create Model
            var createModel = new Promise(function(resolve, reject) {
                try {
                    var model = part.createModel(that.getDataSource());
                    model.once(PartModel.READY, function(model) {
                        if ( model instanceof PartModel) {
                            logger.info('%cPartModel.READY', 'color:red');
                            resolve(model);
                        }
                    });
                } catch(e) {
            ...
            //5. Set contents for the view from the model
            Promise.all([createViewer, createModel]).then(function(results) {
