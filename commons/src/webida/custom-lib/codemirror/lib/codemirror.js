...
(function(mod) {
  ...
  function handleKeyBinding(cm, e) {
    ...
    if (e.shiftKey && !cm.state.keySeq) {
      ...
      var handled = dispatchKey(cm, name, e, function(b) { return doHandleBinding(cm, b); });
      if (handled) e_stopPropagation(e);
      return handled;
    }
  }
  ...
  function handleCharBinding(cm, e, ch) {
      var handled = dispatchKey(cm, "'" + ch + "'", e,
  ...
  CodeMirror.prototype = {
    ...
    scrollToLine: operation(null, function(x, line) {
      this.scrollTo(x, this.heightAtLine(line, "local"));
    }),
    scrollBy: operation(null, function(dx, dy) {
      var scroll = this.getScrollInfo();
      this.scrollTo(scroll.left + dx, scroll.top + dy);
    }),
  ...
  var commands = CodeMirror.commands = {
    ...
    defaultTab: function(cm) {
      if (cm.somethingSelected() && cm.getSelection(false).length > 1)	// multi-line selection
    ...
    scrollUp: function(cm, moveCursorIntoScreen) {
      var scroll = cm.getScrollInfo();
      var line = cm.lineAtHeight(scroll.top, "local");
      var bottom = cm.lineAtHeight(scroll.top + scroll.clientHeight, "local");
      cm.scrollToLine(scroll.left, line - 1);
      if (moveCursorIntoScreen) {
        var cursor = cm.getCursor();
        if (bottom >= 2 && cursor.line >= bottom - 1) {
          cm.setCursor({line: bottom - 2, ch: cursor.ch});
        }
      }
    },
    scrollDown: function(cm, moveCursorIntoScreen) {
      var scroll = cm.getScrollInfo();
      var line = cm.lineAtHeight(scroll.top, "local");
      cm.scrollToLine(scroll.left, line + 1);
      if (moveCursorIntoScreen) {
  ...
  StringStream.prototype = {
    ...
    match: function(pattern, consume, caseInsensitive) {
      if (typeof pattern == "string") {
        ...
        if (pattern.source.substring(0, 2) === '\\b') {
          if (!(this.pos === 0 || this.string.charAt(this.pos - 1).match(/\W/))) {
            return null;
          }
        }
