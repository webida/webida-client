...
CodeMirror.defineMode("xml", function(config, parserConfig) {
  ...
  function attrState(type, _stream, state) {
    if (type == "word") {
      setStyle = "attribute";
      state.attrName = _stream.string.substring(_stream.start, _stream.pos);
      return attrEqState;
  ...
  function attrValueState(type, stream, state) {
    ...
    state.attrName = undefined;
    if (/\bstring\b/.test(type)) {
      if (Kludges.resourceAttributes[state.tagName] && Kludges.resourceAttributes[state.tagName][attrName]) {
        setStyle = "link";
      }
