...
define(['./plugin',
...
], function (editors, workbench, vm, topic, _) {
    'use strict';

    function undo() {
        if (editors.currentFile && editors.currentFile.editor && editors.currentFile
            .editor.editor) {
            // currentFile focus
        ...
        if (editors.currentFile && editors.currentFile.editor && editors.currentFile
            ...
            if (selected) {
                ...
                if (info.text && info.text.length > 0) {
                    editor.replaceRange('', {line: pos.line, ch: 0}, {line: pos.line, ch: info.text.length});
                } else {
        ...
        if (editors.currentFile && editors.currentFile.editor && editors.currentFile
            ...
            var moveUp = function (pos) {
                ...
                editor.replaceRange(desLineText,
                                    {line: srcLineNum, ch: 0},
                                    {line: srcLineNum, ch: srcLineText.length},
                                    '+input');
                editor.replaceRange(srcLineText,
                                    {line: desLineNum, ch: 0},
                                    {line: desLineNum, ch: desLineText.length},
                                    '+input');
                editor.setCursor({
                    line: desLineNum,
                    ch: pos.ch
    ...
    function lineMoveDown() {
        if (editors.currentFile && editors.currentFile.editor && editors.currentFile
            ...
            var moveDown = function (pos) {
                ...
                editor.replaceRange(desLineText,
                                    {line: srcLineNum, ch: 0}, {line: srcLineNum, ch: srcLineText.length}, '+input');
                editor.replaceRange(srcLineText,
                                    {line: desLineNum, ch: 0}, {line: desLineNum, ch: desLineText.length}, '+input');
                editor.setCursor({
                    line: desLineNum,
                    ch: pos.ch
                });
            ...
            if (selected) {
                ...
                editor.replaceRange(selected,
                                    {line: startPos.line + 1, ch: 0},
                                    {line: desLineNum, ch: desLineText.length},
                                    '+input');
                editor.replaceRange(desLineText,
                                    {line: startPos.line, ch: 0},
                                    {line: startPos.line, ch: desLineText.length},
                                    '+input');
            ...
            getBeautifier(editor, function (beautifier, options) {
                if (beautifier) {
                    ...
                    if (startPos.ch !== 0) {
                        ...
                        startPos = { line: startPos.line, ch: 0 };
                    }
                    if (endPosInfo.text.length !== endPos.ch) {
                        endPos = { line: endPos.line, ch: endPosInfo.text.length };
                    }

                    editor.replaceRange(beautifier(editor.getRange(startPos, endPos), options),
                                        startPos, endPos);
                }
            });
        }
                    ...
                    editor.operation(function () {
                        if (mode.blockCommentStart) {
                            ...
                            editor.replaceRange(anchorComment + '\n', { line: cursor.line, ch: 0},
                                                { line: cursor.line, ch: 0}, '+beautifyAll');
                        }

                        var startPos = { line: editor.firstLine(), ch: 0 };
                        var lastLine, endPos = { line: (lastLine = editor.lastLine()),
                                                ch: editor.getLine(lastLine).length };
                        editor.replaceRange(beautifier(editor.getValue(), options),
                                            startPos, endPos, '+beautifyAll');
                        ...
                        if (mode.blockCommentStart) {
                            ...
                            if (j >= 0) {
                                var token = editor.getTokenAt({ line: i, ch: j }, true);
    ...
    function moveToOtherTabContainer() {
        ...
        if (vc && view) {
            ...
            console.assert(i >= 0);
            var targetView = (i < viewContainers.length - 1 ?
                              viewContainers[i + 1] : viewContainers[0]);
    ...
    function closeOtherFiles() {
        ...
        _.each(opened, function (file) {
            if (file.path !== curFilePath) {
                editors.closeFile({ path: file.path });
            }
        });
    }
